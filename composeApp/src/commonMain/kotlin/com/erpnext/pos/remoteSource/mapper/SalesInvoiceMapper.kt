...
fun SalesInvoiceDto.toEntity(): SalesInvoiceWithItemsAndPayments {
  ...
  val financials =
      resolveInvoiceFinancialsFromRemote(
  ...
  val invoiceEntity =
      SalesInvoiceEntity(
          ...
          baseTotalTaxesAndCharges =
              resolveBaseAmount(totalTaxesAndCharges, baseTotalTaxesAndCharges),
  ...
  val paymentsEntity =
      payments.map { dto ->
        POSInvoicePaymentEntity(
            ...
            // La moneda real del pago es la moneda de la factura; party_account_currency suele ser
            // USD.
...
private data class ResolvedInvoiceFinancials(val paidAmount: Double, val outstandingAmount: Double)
...
private fun resolveInvoiceFinancialsFromRemote(
  ...
  val outstandingProvided = outstandingAmount?.coerceAtLeast(0.0)?.coerceAtMost(total)
