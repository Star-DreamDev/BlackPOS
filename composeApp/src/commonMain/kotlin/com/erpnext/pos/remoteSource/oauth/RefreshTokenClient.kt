...
suspend fun refreshAuthToken(
  ...
  val requestTrace =
      "site=${currentSite.trim()} tokenUrl=${config.tokenUrl} " +
  ...
  try {
    val params =
        Parameters.build {
              ...
              oauthConfig.redirectUrl.takeIf { it.isNotBlank() }?.let { append("redirect_uri", it) }
            }
            .formUrlEncode()
    AppLogger.info("OAuth refresh request -> $requestTrace")
    val response =
        client.post(config.tokenUrl) {
