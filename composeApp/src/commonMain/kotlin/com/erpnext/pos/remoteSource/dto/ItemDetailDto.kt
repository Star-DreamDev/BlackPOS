...
data class ItemDetailDto(
    ...
    @SerialName("rate") val price: Double = 0.0, // Precio calculado con rules
    ...
    @Serializable(with = IntAsBooleanSerializer::class) val isService: Boolean = false, // Inferido
    @SerialName("is_stock_item")
    @Serializable(with = IntAsBooleanSerializer::class)
    val isStocked: Boolean = false,
    @SerialName("stock_uom") val stockUom: String = "",
    @SerialName("brand") val brand: String = "",
    @SerialName("standard_rate") val standardRate: Double = 0.0, // Fallback si no rate
