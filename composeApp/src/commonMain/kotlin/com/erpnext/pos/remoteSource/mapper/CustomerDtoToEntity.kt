...
fun POSClosingEntryDto.toEntity(
    name: String,
    pendingSync: Boolean = true,
    posProfile: String,
    user: String,
    periodStartDate: String,
): POSClosingEntryEntity {
...
fun CustomerDto.resolveReceivableAccount(company: String?): CustomerReceivableAccountDto? {
  ...
  if (!normalizedCompany.isNullOrBlank()) {
    receivableAccounts
        .firstOrNull { account -> account.company?.trim()?.lowercase() == normalizedCompany }
        ?.let {
          return it
        }
  }
...
fun CustomerEntity.toBO(): CustomerBO =
    CustomerBO(
        name = name,
        customerName = customerName,
        territory = territory,
