...
data class InvoicePdfDownload(val fileName: String, val bytes: ByteArray)
...
class SalesInvoiceRemoteSource(
  ...
  suspend fun fetchStockForItems(warehouse: String, itemCodes: List<String>): Map<String, Double> =
      apiService.fetchStockForItems(warehouse, itemCodes)
  ...
  suspend fun fetchReturnInvoices(
    ...
    return names.mapNotNull { name -> fetchInvoice(name) }
  }

  suspend fun createInvoice(invoice: SalesInvoiceDto): SalesInvoiceDto =
      apiService.createSalesInvoice(invoice)

  suspend fun cancelInvoice(name: String) = apiService.cancelSalesInvoice(name)
  ...
  suspend fun downloadInvoicePdf(invoiceName: String): InvoicePdfDownload {
    ...
    return InvoicePdfDownload(fileName = payload.fileName, bytes = payload.bytes)
  }
