...
class OpeningEntrySyncRepository(
  ...
  suspend fun pushPendingWithReport(): PushQueueReport {
    ...
    pending.forEach { candidate ->
      ...
      runCatching {
            val remoteName =
                candidate.link.remoteOpeningEntryName
                    ?: run {
            ...
            if (remote != null && remote.name.isNotBlank()) {
              ...
              conflicts +=
                  PushSyncConflict(
                      ...
                      remoteId = remoteName,
                      reason =
                          "Apertura local pendiente ya existía en remoto; se adoptó el registro remoto.",
                  )
              return@onFailure
            }
  ...
  suspend fun repairActiveOpenings() {
    ...
    activeCashboxes.forEach { wrapper ->
      ...
      val localOpeningName =
          wrapper.details.firstOrNull()?.posOpeningEntry
      ...
      if (link == null) {
        val pendingSync =
            remoteName.isNullOrBlank() || openingEntry.pendingSync || cashbox.pendingSync
  ...
  private suspend fun updateOpeningEntryRefs(localName: String, remoteName: String) {
    ...
    val remoteSyncedAffected =
        affected.count { invoice ->
  ...
  private suspend fun resolveRemoteOpenSession(
      candidate: PendingOpeningEntrySync
  ): POSOpeningEntrySummaryDto? {
    val user =
        candidate.openingEntry.user?.trim()?.takeIf { it.isNotBlank() }
    ...
    return runCatching { posOpeningRepository.getOpenSession(user, profile) }.getOrNull()
  }
}
