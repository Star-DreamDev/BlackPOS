...
class InventoryAlertRepository(
  ...
  data class ReorderInfo(val reorderLevel: Double?, val reorderQty: Double?)

  suspend fun loadAlerts(warehouseId: String, limit: Int): List<InventoryAlert> {
    val items = itemDao.getAllItems().filter { it.isStocked && !it.isDeleted }

    val itemCodes = items.map { it.itemCode }
    val projectedQtyMap = emptyMap<String, Double>()

    val localReorderMap =
        itemReorderDao.getByItems(warehouseId = warehouseId, itemIds = itemCodes).associate {
          it.itemId to ReorderInfo(it.reorderLevel, it.reorderQty)
        }
        ...
        items.mapNotNull { item ->
          ...
          val status =
              when {
                ...
                level != null && level > 0.0 && qty <= (level * 0.35) ->
                    InventoryAlertStatus.CRITICAL
