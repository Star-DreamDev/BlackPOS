...
class SalesInvoiceRepository(
  ...
  override suspend fun getInvoiceDetail(invoiceId: String): SalesInvoiceBO {
    val invoices =
        localSource.getInvoiceByName(invoiceId)?.invoice
  ...
  suspend fun applyLocalReturnAdjustment(invoiceName: String, returnTotal: Double) {
    ...
    val rate =
        when {
          local.conversionRate != null && (local.conversionRate ?: 0.0) > 0.0 ->
    ...
    val basePaid =
        when {
          ...
          local.basePaidAmount != null ->
              roundToCurrency(
                  (local.basePaidAmount ?: 0.0).coerceAtMost(baseGrand ?: Double.MAX_VALUE)
              )
          else -> null
        }
    val newStatus =
  ...
  suspend fun saveInvoiceLocallyPending(localInvoiceName: String, dto: SalesInvoiceDto) {
  ...
  override suspend fun applyLocalPayment(
    ...
    val paidDeltaReceivable =
        normalizedPayments.sumOf { payment ->
    ...
    val rateToBase =
        when {
          ...
          invoice.baseGrandTotal != null && invoice.grandTotal > 0.0 ->
              invoice.baseGrandTotal?.let { it / invoice.grandTotal }
    ...
    invoice.basePaidAmount =
        rateToBase?.let { roundToCurrency(cappedPaid * it) } ?: invoice.basePaidAmount
    // Keep outstanding_amount in receivable/invoice party currency; do not overwrite it with base
    // currency.
    invoice.status =
        when {
  ...
  @OptIn(ExperimentalTime::class)
  suspend fun updateLocalInvoiceFromRemote(localInvoiceName: String, remote: SalesInvoiceDto) {
    ...
    val remoteTotalChanged = localInvoice?.let { abs(it.grandTotal - remoteTotal) > 0.01 } == true
    ...
    val resolvedStatus =
        if (trustRemoteFinancials) remote.status ?: "Draft"
    ...
    localSource.updateFromRemote(
        ...
        baseTotalTaxesAndCharges =
            resolveBaseAmount(remote.totalTaxesAndCharges, remote.baseTotalTaxesAndCharges),
        baseGrandTotal = resolveBaseAmount(remote.grandTotal, remote.baseGrandTotal),
        baseRoundingAdjustment =
            resolveBaseAmount(remote.roundingAdjustment, remote.baseRoundingAdjustment),
        ...
        posOpeningEntry =
            remote.posOpeningEntry?.takeIf { it.isNotBlank() } ?: localInvoice?.posOpeningEntry,
  ...
  override suspend fun createRemoteInvoice(invoice: SalesInvoiceDto): SalesInvoiceDto {
    ...
    if (!allowNegativeStock && invoice.items.isNotEmpty()) {
      val warehouse =
          invoice.items.firstOrNull { !it.warehouse.isNullOrBlank() }?.warehouse ?: ctx?.warehouse
      if (!warehouse.isNullOrBlank()) {
        ...
        val stock =
            runCatching { remoteSource.fetchStockForItems(warehouse, itemTotals.keys.toList()) }
                .getOrNull()
        if (stock != null) {
          val shortage =
              itemTotals.entries.firstOrNull { (code, qty) ->
                val available = stock[code] ?: 0.0
                qty > available + 0.0001
              }
          if (shortage != null) {
            throw IllegalStateException("Item ${shortage.key} no tiene stock en $warehouse.")
          }
        }
      }
    }
  ...
  override suspend fun syncPendingInvoices(): PushQueueReport {
    ...
    pending.forEach { invoice ->
      try {
        when {
          ...
          invoice.invoice.invoiceName?.startsWith("LOCAL-", ignoreCase = true) == true -> {
    ...
    return PushQueueReport(hasChanges = hasChanges, conflicts = conflicts)
  }
  ...
  private suspend fun handleLocalInvoice(
    ...
    val allowNegativeStock = ctx?.allowNegativeStock == true
    val openingFromPayments =
        invoice.payments.mapNotNull { it.posOpeningEntry?.trim() }.firstOrNull { it.isNotBlank() }
    val resolvedProfile = invoice.invoice.profileId?.takeIf { it.isNotBlank() } ?: ctx?.profileName
    val resolvedOpening =
        invoice.invoice.posOpeningEntry?.takeIf { it.isNotBlank() } ?: openingFromPayments
    ...
    val normalizedInvoice =
        invoice.invoice.copy(profileId = resolvedProfile, posOpeningEntry = resolvedOpening)
    val normalizedPayments =
    ...
    if (
        normalizedInvoice.profileId != invoice.invoice.profileId ||
    ...
    if (!allowNegativeStock && !warehouse.isNullOrBlank() && invoice.items.isNotEmpty()) {
      ...
      if (stock != null) {
        ...
        if (shortage != null) {
          markAsFailed(localName)
          AppLogger.warn("handleLocalInvoice: insufficient stock for ${shortage.key} in $warehouse")
          return null
        }
      }
    }
    ...
    val existingName =
        remoteSource.findExistingInvoiceName(
      ...
      val rate =
          when {
            receivableCurrency.equals(baseCurrency, ignoreCase = true) -> 1.0
                ...
                context.resolveExchangeRateBetween(
                    receivableCurrency,
                    baseCurrency,
                    allowNetwork = false,
                )
  ...
  private fun resolveRemoteFinancials(remote: SalesInvoiceDto): ResolvedRemoteFinancials {
    ...
    val outstandingProvided = remote.outstandingAmount?.coerceAtLeast(0.0)?.coerceAtMost(total)
    ...
    val statusSuggestsPayment =
        remote.status?.trim()?.lowercase() in
            setOf("paid", "partly paid", "partly paid and discounted", "credit note issued")

    if (
        outstandingProvided != null &&
            inferredPaidFromOutstanding > tolerance &&
            (paidMissing ||
                statusSuggestsPayment ||
  ...
  private fun normalizeLocalPaymentStatus(invoice: SalesInvoiceEntity): SalesInvoiceEntity {
    ...
    return if (invoice.status == normalizedStatus) invoice
    else invoice.copy(status = normalizedStatus)
  }
  ...
  private suspend fun enrichPaymentsWithAccount(dto: SalesInvoiceDto): SalesInvoiceDto {
    ...
    val definitions =
        runCatching { modeOfPaymentDao.getAllModes(ctx.company) }.getOrElse { emptyList() }
  ...
  suspend fun getOutstandingInvoicesForCustomer(customerName: String): List<SalesInvoiceBO> {
    val posProfile = context.requireContext().profileName
    val remoteInvoices =
        runCatching { remoteSource.fetchOutstandingInvoicesForCustomer(customerName, posProfile) }
            .getOrNull()
  ...
  fun getOutstandingInvoicesForCustomerLocalPaged(
      customerName: String
  ): Flow<PagingData<SalesInvoiceBO>> {
    return Pager(PagingConfig(pageSize = 30, prefetchDistance = 10)) {
  ...
  private fun normalizeBaseOutstanding(
    ...
    val invoice = payload.invoice.copy(basePaidAmount = payload.invoice.paidAmount)
    ...
    val mergedInvoice =
        payload.invoice.copy(
            ...
            partyAccountCurrency =
                payload.invoice.partyAccountCurrency?.takeIf { it.isNotBlank() }
  ...
  private fun ensurePosOpeningEntry(
    ...
    val normalizedInvoice =
        payload.invoice.copy(profileId = invoiceProfile, posOpeningEntry = fallbackOpening)
    val normalizedPayments =
  ...
  private suspend fun syncPaymentEntriesForPeriod(days: Int = 90) {
    ...
    val currencySpecs = buildCurrencySpecs()
    val entries =
        runCatching { remoteSource.fetchPaymentEntries(startDate) }.getOrNull() ?: emptyList()
    if (entries.isEmpty()) return
    val touchedInvoices = mutableSetOf<String>()

    entries.forEach { entry ->
      val entryName = entry.name ?: return@forEach
      val detailed = runCatching { remoteSource.fetchPaymentEntry(entryName) }.getOrNull() ?: entry
      ...
      detailed.references.forEach { ref ->
        ...
        val localInvoice = localSource.getInvoiceByName(invoiceName)?.invoice ?: return@forEach
        ...
        val match =
            payments.firstOrNull { payment ->
              ...
              val modeMatch =
                  mode.isNullOrBlank() || payment.modeOfPayment.equals(mode, ignoreCase = true)
  ...
  private suspend fun ensurePosOpeningEntry(dto: SalesInvoiceDto): SalesInvoiceDto {
    ...
    val openingEntryId =
        context.getActiveCashboxWithDetails()?.cashbox?.openingEntryId?.takeIf { it.isNotBlank() }
            ?: return dto
