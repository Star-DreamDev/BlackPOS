...
class ClosingEntrySyncRepository(
  ...
  suspend fun reconcileRemoteClosingsForActiveCashboxes(): Boolean {
    ...
    active.forEach { wrapper ->
      ...
      val remoteOpeningName =
          openingEntryLinkDao.getRemoteOpeningEntryName(cashbox.localId)
      ...
      val remoteClosing =
          runCatching { api.getPOSClosingEntriesForOpening(remoteOpeningName).firstOrNull() }
              .onFailure {
                AppLogger.warn(
      ...
      val shiftInvoices = buildList {
        ...
        if (isEmpty()) {
          val localOpening =
              openingEntryLinkDao.getByCashboxId(cashbox.localId)?.localOpeningEntryName
      ...
      shiftInvoices.forEach { invoice ->
        ...
        if (
            invoice.posOpeningEntry != remoteOpeningName || invoice.profileId != cashbox.posProfile
        ) {
      ...
      val modeCurrency =
          runCatching { paymentMethodLocalRepository.getMethodsForProfile(cashbox.posProfile) }
              .getOrElse { emptyList() }
              .associate { method ->
      ...
      upsertClosingLinkForCashbox(
          cashbox = cashbox,
          remoteOpeningName = remoteOpeningName,
      ...
      if (remoteClosing != null) {
        conflicts +=
            PushSyncConflict(
                ...
                localId =
                    localClosingName?.takeIf { it.isNotBlank() } ?: "cashbox-${cashbox.localId}",
  ...
  private suspend fun upsertClosingLinkForCashbox(
      ...
      remoteOpeningName: String?,
      remoteClosingName: String,
      pendingSync: Boolean,
  ) {
    val existing = openingEntryLinkDao.getByCashboxId(cashbox.localId)
    if (existing != null) {
      val remoteOpening = remoteOpeningName?.trim()?.takeIf { it.isNotBlank() }
      if (
      ...
      if (!remoteClosingName.equals(existing.remoteClosingEntryName, ignoreCase = true)) {
        openingEntryLinkDao.updateRemoteClosingName(existing.id, remoteClosingName)
      }
      return
    }

    val localOpening =
        cashbox.openingEntryId?.trim()?.takeIf { it.isNotBlank() }
            ?: remoteOpeningName?.trim()?.takeIf { it.isNotBlank() }
    if (localOpening.isNullOrBlank()) {
      AppLogger.warn(
          "ClosingEntrySyncRepository: missing opening link for cashbox ${cashbox.localId}"
      )
      return
    }
    ensureOpeningEntryForLink(
        cashbox = cashbox,
        openingEntryName = localOpening,
        pendingSync = pendingSync,
    )
    ...
    openingEntryLinkDao.insert(
        POSOpeningEntryLinkEntity(
            ...
            remoteClosingEntryName = remoteClosingName,
            pendingSync = pendingSync,
        )
    )
  }

  private suspend fun ensureOpeningEntryForLink(
      cashbox: CashboxEntity,
      openingEntryName: String,
      pendingSync: Boolean,
  ) {
    if (openingEntryDao.getByName(openingEntryName) != null) return
