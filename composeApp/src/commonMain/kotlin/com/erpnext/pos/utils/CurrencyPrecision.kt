...
@Serializable data class CurrencyPrecisionEntry(val precision: Int, val cashScale: Int)
...
object CurrencyPrecisionResolver {
  ...
  fun defaultPrecision(): Int {
    return snapshot.currencyPrecision ?: snapshot.floatPrecision ?: 2
  }
  ...
  fun roundWithScale(value: Double, scale: Int): Double {
    if (!value.isFinite()) return value
    val rounded =
        if (isBankersRounding(snapshot.roundingMethod)) {
