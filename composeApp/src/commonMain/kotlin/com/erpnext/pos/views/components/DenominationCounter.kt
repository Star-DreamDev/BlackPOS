...
@Composable
fun CurrencySelector(currencies: List<String>, selected: String, onSelect: (String) -> Unit) {
  if (currencies.size <= 1) return
  Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.End) {
    currencies.forEach { code ->
      FilterChip(selected = code == selected, onClick = { onSelect(code) }, label = { Text(code) })
    }
  }
}
  ...
  Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
    ...
    Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
      denominations.forEach { denom ->
        DenominationRow(denom = denom, onCountChange = onCountChange, formatAmount = formatAmount)
      }
    }
  }
}
