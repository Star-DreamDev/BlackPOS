...
@Composable
private fun ActivityItemCard(item: ActivityEntry, onClick: () -> Unit) {
  val strings = LocalAppStrings.current
  val indicatorColor =
      when (item.priority) {
...
private fun EmptyActivityState(strings: com.erpnext.pos.localization.AppStrings) {
  ...
  Box(
      modifier = Modifier.fillMaxSize().padding(horizontal = 8.dp, vertical = 20.dp),
    ...
    ElevatedCard(
      ...
      Column(
        ...
        Box(
            modifier =
                Modifier.size(56.dp).background(scheme.primary.copy(alpha = 0.10f), CircleShape),
            contentAlignment = Alignment.Center,
        ) {
          Icon(
