...
data class SalesFlowActionItem(val label: String, val onClick: () -> Unit)
...
fun SalesFlowContextSummary(context: SalesFlowContext?) {
  ...
  val sourceInfo =
      sourceLabel?.let { label -> context.sourceId?.let { id -> "$label (ID: $id)" } ?: label }
...
fun SalesFlowActionButtons(
  ...
  Column(modifier = Modifier.fillMaxWidth(), verticalArrangement = Arrangement.spacedBy(8.dp)) {
    Button(onClick = primary.onClick, modifier = Modifier.fillMaxWidth()) { Text(primary.label) }
    secondary.forEach { action ->
      Button(onClick = action.onClick, modifier = Modifier.fillMaxWidth()) { Text(action.label) }
    }
  }
}
