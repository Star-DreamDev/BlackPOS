...
@Composable
fun CustomerRoute(coordinator: CustomerCoordinator = rememberCustomerCoordinator()) {
  val uiState by coordinator.screenStateFlow.collectAsState(CustomerState.Loading)
  val customersPagingFlow by
      coordinator.customersPagingFlow.collectAsState(flowOf(PagingData.empty()))
  val outstandingInvoicesPagingFlow by
      coordinator.outstandingInvoicesPagingFlow.collectAsState(
          flowOf(PagingData.empty<SalesInvoiceBO>())
      )
  val historyInvoicesPagingFlow by
      coordinator.historyInvoicesPagingFlow.collectAsState(
          flowOf(PagingData.empty<SalesInvoiceBO>())
      )
  ...
  LaunchedEffect(Unit) { actions.fetchAll() }
