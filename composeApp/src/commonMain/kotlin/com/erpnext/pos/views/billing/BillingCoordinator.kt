...
class BillingCoordinator(val viewModel: BillingViewModel) {
  ...
  fun onManualDiscountPercentChanged(value: String) =
      viewModel.onManualDiscountPercentChanged(value)
  ...
  fun onLoadSourceDocuments(sourceType: SalesFlowSource) = viewModel.loadSourceDocuments(sourceType)
...
fun rememberBillingCoordinator(): BillingCoordinator {
  ...
  return remember(viewModel) { BillingCoordinator(viewModel = viewModel) }
}
...
fun rememberBillingLabCoordinator(owner: ViewModelStoreOwner): BillingCoordinator {
  ...
  val viewModel: BillingViewModel = koinViewModel(viewModelStoreOwner = owner)
