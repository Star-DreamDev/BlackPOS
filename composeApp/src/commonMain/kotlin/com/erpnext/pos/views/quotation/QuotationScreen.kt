...
@Composable
fun QuotationScreen(state: QuotationState, action: QuotationAction, context: SalesFlowContext?) {
  Scaffold { padding ->
    Column(
          ...
          Card(
            ...
            Column(
              ...
              items.forEach { item ->
                Text(
                    text =
                        "${item.name} | Qty: ${item.qty} | Rate: ${item.rate} | Amount: ${item.amount}",
          ...
          SalesFlowActionButtons(
              primary =
                  SalesFlowActionItem(
                      ...
                      onClick = {
                        action.onCreateSalesOrder(referenceId.takeIf { it.isNotBlank() })
                      },
                  ),
              secondary =
                  listOf(
...
private data class DraftItem(val name: String, val qty: Double, val rate: Double) {
