...
@Composable
fun QuotationRoute(coordinator: QuotationCoordinator = rememberQuotationCoordinator()) {
  ...
  return remember(coordinator, navManager, salesFlowStore, salesContext) {
    QuotationAction(
        ...
        onCreateSalesOrder = { sourceId ->
          salesFlowStore.set(
              (salesContext ?: SalesFlowContext()).withSource(SalesFlowSource.Quotation, sourceId)
          )
