...
class CancelSalesInvoiceUseCase(
  ...
  override suspend fun useCaseFunction(input: CancelSalesInvoiceInput): CancelSalesInvoiceResult {
    val invoice =
        invoiceRepository.getInvoiceByName(input.invoiceName)
  ...
  private suspend fun handleCancel(
    ...
    val hasPayments = invoice.invoice.paidAmount > 0.0 || invoice.payments.any { it.amount > 0.0 }
  ...
  private suspend fun handleReturn(
    ...
    invoice.invoice.invoiceName?.let { originalName ->
      returnLedgerPreferences.recordReturn(
          originalName,
          returnItems.associate { it.itemCode to abs(it.qty) },
      )
    }
  ...
  private suspend fun buildRemainingReturnItems(
    ...
    val baseItems =
        remoteParent?.items?.takeIf { it.isNotEmpty() } ?: invoice.items.map { it.toDto(parent) }
    val localReturnedByItem = returnLedgerPreferences.returnedByItem(originalName)
    val returnInvoices = invoiceRepository.fetchRemoteReturnInvoices(returnAgainst = originalName)
    val remoteReturnedByItem = mutableMapOf<String, Double>()
    returnInvoices.forEach { credit ->
      credit.items.forEach { item ->
        val qty = abs(item.qty)
        if (qty > 0.0) {
          remoteReturnedByItem[item.itemCode] = (remoteReturnedByItem[item.itemCode] ?: 0.0) + qty
        }
      }
    }
    val returnedByItem =
  ...
  private fun createReturnItemFromDto(
    ...
    val perUnit =
        if (item.qty != 0.0) item.amount / item.qty else item.rate.takeIf { it != 0.0 } ?: 0.0
    val absAmount = abs(perUnit) * qtyToReturn
    return item.copy(qty = -qtyToReturn, amount = -absAmount)
  }
  ...
  private suspend fun createReturnPaymentEntries(
    ...
    val originalInvoiceName = invoice.invoice.invoiceName?.takeIf { it.isNotBlank() } ?: return
