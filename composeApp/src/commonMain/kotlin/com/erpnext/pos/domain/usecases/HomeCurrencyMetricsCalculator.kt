  ...
  fun build(input: Input): List<CurrencyHomeMetric> {
    ...
    return currencies.map { currency ->
      ...
      val marginTodayPercent =
          marginTodayValue?.takeIf { totalSalesToday > 0.0 }?.let { (it / totalSalesToday) * 100.0 }
      val marginLast7Percent =
          marginLast7Value?.takeIf { salesLast7 > 0.0 }?.let { (it / salesLast7) * 100.0 }
