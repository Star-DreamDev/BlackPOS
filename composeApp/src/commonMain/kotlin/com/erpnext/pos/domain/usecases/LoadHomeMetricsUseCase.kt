...
class LoadHomeMetricsUseCase(private val salesInvoiceDao: SalesInvoiceDao) :
  ...
  override suspend fun useCaseFunction(input: HomeMetricInput): HomeMetrics {
    ...
    val currencySummaries =
        if (!trendOpeningEntryId.isNullOrBlank()) {
          salesInvoiceDao.getShiftTodaySummaryByCurrency(todayString, trendOpeningEntryId).map { row
            ->
    ...
    val dailyTotalsByCurrency =
        salesInvoiceDao.getDailySalesTotalsByCurrency(
            startDate = startDate14.toString(),
            endDate = today.toString(),
            openingEntryId = trendOpeningEntryId,
        )
    val marginTodayByCurrency =
        salesInvoiceDao.getEstimatedMarginTotalByCurrency(
            startDate = todayString,
            endDate = todayString,
            openingEntryId = dayOpeningEntryId,
        )
    val marginLast7ByCurrency =
        salesInvoiceDao.getEstimatedMarginTotalByCurrency(
            startDate = last7Start.toString(),
            endDate = today.toString(),
            openingEntryId = trendOpeningEntryId,
        )
    val costedTodayByCurrency =
        salesInvoiceDao.countItemsWithCostByCurrency(
            startDate = todayString,
            endDate = todayString,
            openingEntryId = dayOpeningEntryId,
        )
    val totalItemsTodayByCurrency =
        salesInvoiceDao.countItemsInRangeByCurrency(
            startDate = todayString,
            endDate = todayString,
            openingEntryId = dayOpeningEntryId,
        )
    ...
    val currencyMetrics =
        HomeCurrencyMetricsCalculator.build(
    ...
    val costedItemsToday =
        salesInvoiceDao.countItemsWithCost(
            startDate = todayString,
            endDate = todayString,
            openingEntryId = dayOpeningEntryId,
        )
    val totalItemsToday =
        salesInvoiceDao.countItemsInRange(
            startDate = todayString,
            endDate = todayString,
            openingEntryId = dayOpeningEntryId,
        )
    val costCoveragePercent =
        if (totalItemsToday > 0) {
          (costedItemsToday.toDouble() / totalItemsToday.toDouble()) * 100.0
        } else {
          null
        }
    val marginToday =
        if (costedItemsToday > 0) {
    ...
    val marginTodayPercent =
        marginToday?.takeIf { totalSalesToday > 0.0 }?.let { (it / totalSalesToday) * 100.0 }

    val costedItemsLast7 =
        salesInvoiceDao.countItemsWithCost(
            startDate = last7Start.toString(),
            endDate = today.toString(),
            openingEntryId = trendOpeningEntryId,
    ...
    val marginLast7Percent =
        marginLast7?.takeIf { salesLast7 > 0.0 }?.let { (it / salesLast7) * 100.0 }
