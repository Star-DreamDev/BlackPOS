...
data class ObserveHomeLiveShiftMetricsInput(val postingDate: String, val openingEntryId: String)
...
class ObserveHomeLiveShiftMetricsUseCase(private val salesInvoiceDao: SalesInvoiceDao) {
  fun observe(input: ObserveHomeLiveShiftMetricsInput): Flow<HomeLiveShiftMetrics> {
    val summaryFlow =
        salesInvoiceDao.observeShiftTodaySummary(
            date = input.postingDate,
            openingEntryId = input.openingEntryId,
        )
    val byCurrencyFlow =
        salesInvoiceDao.observeShiftTodaySummaryByCurrency(
            date = input.postingDate,
            openingEntryId = input.openingEntryId,
        )
