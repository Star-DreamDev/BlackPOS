  ...
  NavigationRailItem(
      ...
      onClick = {
        if (navController.currentDestination?.route != NavRoute.Activity.path) {
          navController.navigate(NavRoute.Activity.path) { launchSingleTop = true }
        }
      },
      icon = {
        BadgedBox(
...
private fun NavigationRailEntry(
  ...
  if (navRoute == NavRoute.Expenses) {
    ...
    Box {
      NavigationRailItem(
          selected = isSelected,
          onClick = { if (isEnabled) expensesMenuExpanded = true },
          icon = {
            Icon(
                modifier =
        ...
        DropdownMenuItem(
            ...
            onClick = {
              ...
              if (navController.currentDestination?.route != NavRoute.PaymentEntry().path) {
                navController.navigate(NavRoute.PaymentEntry().path) { launchSingleTop = true }
              }
            },
        )
        DropdownMenuItem(
            ...
            onClick = {
              ...
              if (navController.currentDestination?.route != NavRoute.InternalTransfer.path) {
                navController.navigate(NavRoute.InternalTransfer.path) { launchSingleTop = true }
              }
            },
        )
      }
  ...
  NavigationRailItem(
      ...
      onClick = {
        if (isEnabled && navController.currentDestination?.route != navRoute.path) {
          navController.navigate(navRoute.path) { launchSingleTop = true }
        }
      },
      icon = {
        Icon(
