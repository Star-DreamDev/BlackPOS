...
interface DeliveryChargeDao {
  ...
  @Query(
      "UPDATE tabDeliveryCharge SET is_deleted = 1 WHERE is_deleted = 0 AND label NOT IN (:labels)"
  )
  ...
  @Query("DELETE FROM tabDeliveryCharge WHERE is_deleted = 1") suspend fun hardDeleteAllDeleted()
