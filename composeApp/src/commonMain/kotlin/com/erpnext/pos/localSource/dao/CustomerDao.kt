...
interface CustomerDao {
  @Insert(onConflict = OnConflictStrategy.REPLACE) suspend fun insert(customer: CustomerEntity)
  ...
  @Query(
      "SELECT * FROM customers WHERE is_deleted = 0 AND customerName LIKE '%' || :search || '%' ORDER BY customerName ASC"
  )
  ...
  @Query("SELECT COUNT(*) FROM customers WHERE is_deleted = 0") suspend fun count(): Int
  ...
  @Query("UPDATE customers SET is_deleted = 1 WHERE is_deleted = 0") suspend fun softDeleteAll()

  @Query("DELETE FROM customers WHERE is_deleted = 1") suspend fun hardDeleteAllDeleted()
