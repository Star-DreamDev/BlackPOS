...
interface PosProfileLocalDao {
  ...
  @Query("SELECT COUNT(*) FROM tabPosProfileLocal WHERE is_deleted = 0") suspend fun countAll(): Int

  @Query("UPDATE tabPosProfileLocal SET is_deleted = 1 WHERE is_deleted = 0")
  suspend fun softDeleteAll()

  @Query("DELETE FROM tabPosProfileLocal WHERE is_deleted = 1") suspend fun hardDeleteAllDeleted()

  @Query(
      "UPDATE tabPosProfileLocal SET is_deleted = 1 WHERE is_deleted = 0 AND profile_name NOT IN (:profileNames)"
  )
  suspend fun softDeleteNotIn(profileNames: List<String>)

  @Query(
      "DELETE FROM tabPosProfileLocal WHERE is_deleted = 1 AND profile_name NOT IN (:profileNames)"
  )
