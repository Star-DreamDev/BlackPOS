...
interface CustomerGroupDao {
  ...
  @Query(
      "SELECT * FROM tabCustomerGroup WHERE is_deleted = 0 ORDER BY customer_group_name ASC, name ASC"
  )
  ...
  @Query("DELETE FROM tabCustomerGroup WHERE is_deleted = 1") suspend fun hardDeleteAllDeleted()
