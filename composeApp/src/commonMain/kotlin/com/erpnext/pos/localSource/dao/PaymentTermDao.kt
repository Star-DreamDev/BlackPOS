...
interface PaymentTermDao {
  ...
  @Query(
      "UPDATE tabPaymentTerm SET is_deleted = 1 WHERE is_deleted = 0 AND payment_term_name NOT IN (:names)"
  )
  ...
  @Query("DELETE FROM tabPaymentTerm WHERE is_deleted = 1") suspend fun hardDeleteAllDeleted()
