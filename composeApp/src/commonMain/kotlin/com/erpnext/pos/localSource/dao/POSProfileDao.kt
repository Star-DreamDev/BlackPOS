...
interface POSProfileDao {
  ...
  @Update suspend fun update(profile: POSProfileEntity): Int

  @Query("UPDATE tabPosProfile SET is_deleted = 1 WHERE is_deleted = 0") suspend fun softDeleteAll()

  @Query("DELETE FROM tabPosProfile WHERE is_deleted = 1") suspend fun hardDeleteAllDeleted()

  @Query(
      "UPDATE tabPosProfile SET is_deleted = 1 WHERE is_deleted = 0 AND profile_name NOT IN (:profileNames)"
  )
