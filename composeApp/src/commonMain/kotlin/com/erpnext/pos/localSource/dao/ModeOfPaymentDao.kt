...
interface ModeOfPaymentDao {
  ...
  @Query(
      "SELECT MAX(last_synced_at) FROM tabModeOfPayment WHERE company = :company AND is_deleted = 0"
  )
  suspend fun getLastSyncedAt(company: String): Long?

  @Query(
      "UPDATE tabModeOfPayment SET is_deleted = 1 WHERE is_deleted = 0 AND company = :company AND name NOT IN (:names)"
  )
  suspend fun softDeleteNotIn(company: String, names: List<String>)

  @Query(
      "DELETE FROM tabModeOfPayment WHERE is_deleted = 1 AND company = :company AND name NOT IN (:names)"
  )
