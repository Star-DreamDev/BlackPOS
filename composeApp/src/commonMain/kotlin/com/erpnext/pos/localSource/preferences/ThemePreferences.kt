...
class ThemePreferences(private val store: ConfigurationStore) {
  companion object {
  ...
  val theme: Flow<AppColorTheme> =
      store.observeRaw(themeKey).map { value ->
        val key = value ?: defaultTheme
        AppColorTheme.entries.firstOrNull { it.name == key } ?: AppColorTheme.Noir
      }

  val themeMode: Flow<AppThemeMode> =
      store.observeRaw(themeModeKey).map { value ->
