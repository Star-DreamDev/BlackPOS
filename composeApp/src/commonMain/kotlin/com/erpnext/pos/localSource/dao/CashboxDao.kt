...
interface CashboxDao {
  ...
  suspend fun updateClosingAmount(cashboxId: Long, modeOfPayment: String, closingAmount: Double)
  ...
  suspend fun decreaseOpeningAmount(cashboxId: Long, modeOfPayment: String, amount: Double): Int
  ...
  suspend fun getOpeningAmountForMode(cashboxId: Long, modeOfPayment: String): Double?
  ...
  suspend fun increaseOpeningAmount(cashboxId: Long, modeOfPayment: String, amount: Double): Int
  ...
  suspend fun subtractOpeningAmount(cashboxId: Long, modeOfPayment: String, amount: Double): Int
  ...
  @Query(
      "UPDATE tabCashbox SET status = :status, pendingSync = :pendingSync, closingEntryId = :pceId, periodEndDate = :endDate WHERE localId = :localId"
  )
  ...
  @Transaction
  @Query(
      "SELECT * FROM tabCashbox WHERE user = :user AND status = 1 AND posProfile = :posProfile LIMIT 1"
  )
  fun getActiveEntry(user: String, posProfile: String): Flow<CashboxWithDetails?>

  @Transaction
  @Query(
      "SELECT * FROM tabCashbox WHERE openingEntryId = :openingEntryId ORDER BY localId DESC LIMIT 1"
  )
