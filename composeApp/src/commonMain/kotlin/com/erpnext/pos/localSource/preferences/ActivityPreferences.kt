...
class ActivityPreferences(private val store: ConfigurationStore) {
  companion object {
  ...
  val entries: Flow<List<ActivityEntry>> =
      store.observeRaw(activityKey).map { payload ->
        payload
            ?.let { runCatching { json.decodeFromString<List<ActivityEntry>>(it) }.getOrNull() }
  ...
  suspend fun append(entry: ActivityEntry): Boolean {
    ...
    val updated = (listOf(entry) + current).sortedByDescending { it.createdAt }.take(maxEntries)
  ...
  suspend fun upsertFromApi(apiEntries: List<ActivityEntry>): List<ActivityEntry> {
    ...
    val hasChanged =
        merged.size != current.size || merged.zip(current).any { (next, prev) -> next != prev }
    if (hasChanged) {
  ...
  suspend fun markAllRead() {
    val now = Clock.System.now().toEpochMilliseconds()
    val updated =
        currentEntries().map { item -> if (item.isRead) item else item.copy(readAt = now) }
    store.saveRaw(activityKey, json.encodeToString(updated))
  }
