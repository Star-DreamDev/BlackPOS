...
interface CompanyAccountDao {
  ...
  @Query(
      "UPDATE tabCompanyAccount SET is_deleted = 1 WHERE is_deleted = 0 AND name NOT IN (:names)"
  )
  ...
  @Query("DELETE FROM tabCompanyAccount WHERE is_deleted = 1") suspend fun hardDeleteAllDeleted()
