...
interface POSClosingEntryDao {
  ...
  @Insert(onConflict = OnConflictStrategy.REPLACE) suspend fun insert(entry: POSClosingEntryEntity)
  ...
  @Update suspend fun update(entry: POSClosingEntryEntity)

  @Query("DELETE FROM tab_pos_closing_entry WHERE name = :name") suspend fun delete(name: String)

  @Query("DELETE FROM tab_pos_closing_entry") suspend fun clear()
}
