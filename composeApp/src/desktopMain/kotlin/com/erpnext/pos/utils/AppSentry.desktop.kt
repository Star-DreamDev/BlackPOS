...
actual object AppSentry {
  ...
  actual fun init() {
    ...
    try {
      ...
      val debug =
          System.getenv("SENTRY_DEBUG") == "1" || System.getProperty("sentry.debug") == "true"
      ...
      com.erpnext.pos.DesktopLogger.warn("Sentry init failed", t)
    } finally {
      initializing = false
    }
  }

  actual fun breadcrumb(message: String, category: String?, data: Map<String, String>) {
    if (!started) {
  ...
  actual fun capture(
    ...
    com.erpnext.pos.DesktopLogger.info(
