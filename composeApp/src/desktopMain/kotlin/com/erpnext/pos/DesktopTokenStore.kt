...
class DesktopTokenStore(
   ...
   */
  @Volatile private var keyringOk: Boolean = probeKeyring()
  ...
  override suspend fun loadUser(): String? = prefs.get(siteScopedKey("userId"), null)
  ...
  override suspend fun loadAuthInfoByUrl(url: String?, platform: String?): LoginInfo {
    ...
    if (getPlatformName() == "Desktop") {
      sameUrl
          .firstOrNull { it.redirectUrl.contains("127.0.0.1") && !isFallbackCompany(it.company) }
          ?.let {
            return it
          }
      sameUrl
          .firstOrNull { !isFallbackCompany(it.company) }
          ?.let {
            return it
          }
      sameUrl
          .firstOrNull { it.redirectUrl.contains("127.0.0.1") }
          ?.let {
            return it
          }
    }
    sameUrl.firstOrNull()?.let {
      return it
    }
      ...
      mutex.withLock {
        ...
        val mergedCompany =
            mergeCompany(
                ...
                existingCompany =
                    sameUrlEntries.firstOrNull { !isFallbackCompany(it.company) }?.company
                        ?: existing?.company,
            )
        list.add(
            info.copy(
  ...
  override suspend fun getCurrentSite(): String? = prefs.get("current_site", null)
  ...
  override suspend fun loadPkceVerifier(): String? = getSecret("pkce_verifier")
  ...
  override suspend fun loadState(): String? = getSecret("oauth_state")
  ...
  override suspend fun loadRedirectUri(): String? = getSecret("oauth_redirect_uri")
